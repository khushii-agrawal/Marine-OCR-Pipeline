...
REF_PATH = PROJECT_ROOT / "test" / "Test 3" / "FINAL DRAWINGS.xlsm"
OUT_PATH = PROJECT_ROOT / "output" / "Test3_pages_69_197_extracted.xlsm"
...
def load_rows(path, filter_range):
    ...
    for idx, row in enumerate(ws.iter_rows(min_row=3, max_col=MAX_COLS, values_only=True), start=3):
        ...
        if filter_range and (page is None or page < START_PAGE or page > END_PAGE):
            continue
...
def main():
    ...
    ref_rows = load_rows(REF_PATH, filter_range=True)
    out_rows = load_rows(OUT_PATH, filter_range=True)
    ...
    print("=" * 64)
    print("TEST3 PAGE 69-197 ACCURACY REPORT")
    print("=" * 64)
