...
def map_test_page_to_original(test_page):
    ...
    for start, end in ranges:
        count = end - start + 1
        if test_page <= offset + count:
            idx_in_range = test_page - offset - 1
            original_page_idx = start + idx_in_range
            return original_page_idx + 1
        offset += count
    return None
...
def evaluate():
    ...
    # We only want to compare items on pages that were actually in the test split
    test_ranges = [
        (23, 24),
        (59, 61),
        (141, 150),
    ...
    for row_idx, row_vals in enumerate(ref_sheet.iter_rows(min_row=1, max_col=21, values_only=True), start=1):
        ...
        if is_page_in_test_range(page_no):
            ...
            key = (int(page_no), normalize_str(pos_no))
            ref_items[key] = {
    ...
    for row_idx, row_vals in enumerate(out_sheet.iter_rows(min_row=1, max_col=21, values_only=True), start=1):
        ...
        test_page_no = row_vals[12] # M: Page No (index 12)
        orig_page_no = map_test_page_to_original(test_page_no)
        pos_no = row_vals[7] # H: Pos. No. (index 7)
        
        if orig_page_no is not None:
            key = (orig_page_no, normalize_str(pos_no))
            extracted_items[key] = {
                "sub_component": row_vals[1],
                "name_of_spare": row_vals[4],
                "mfg_part_no": row_vals[5],
    ...
    missing_keys = set(ref_items.keys()) - set(extracted_items.keys())
    extra_keys = set(extracted_items.keys()) - set(ref_items.keys())
    ...
    for key in sorted(matched_keys):
        ...
        part_ok = (normalize_str(ref_part) == normalize_str(ext_part))
        if part_ok:
            part_no_matches += 1
    ...
    if missing_keys:
        ...
        for k in sorted(list(missing_keys))[:10]:
            item = ref_items[k]
            print(f"Page {item['page_no']} | Pos {item['pos_no']} | Name: {item['name_of_spare']} | Dwg: {item['drwg_no']}")
            
    if extra_keys:
        print("\n--- SAMPLE EXTRA/FP ITEMS ---")
        for k in sorted(list(extra_keys))[:10]:
            item = extracted_items[k]
            print(f"Page {item['orig_page_no']} | Pos {item['pos_no']} | Name: {item['name_of_spare']} | Dwg: {item['drwg_no']}")
